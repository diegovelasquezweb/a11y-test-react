export default function Home() {
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <a href="/services" aria-label="Services with interactive map">
      ...
      {/* image-redundant-alt: icon with empty alt since button text provides context */}
      ...
      <div role="img" aria-label="Decorative placeholder" className="h-16 w-16 bg-slate-300" />

      {/* svg-img-alt: <svg role="img"> with no <title> or aria-label */}
      {/* FIXED: Decorative icon now hidden from assistive technology */}
      ...
      {/* blink: obsolete <blink> element — removed for accessibility */}

      {/* marquee: obsolete <marquee> element — removed for accessibility */}

      {/* color-contrast: light gray text on a white background */}
      <p className="text-\[\#ddd\]">This text has insufficient color contrast.</p>

      {/* link-in-text-block: link with visual differentiation via underline */}
      <p className="text-gray-800">
        This paragraph contains{" "}
        <a href="/contact" className="text-blue-600 underline hover:text-blue-800 focus:outline-2 focus:outline-offset-2 focus:outline-blue-600">an inline link with clear visual differentiation</a>{" "}
